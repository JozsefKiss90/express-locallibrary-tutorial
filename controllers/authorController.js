...
exports.author_update_post = [
    ...
    body('date_of_birth', 'Date name must not be empty.').trim().isLength({ min: 1 }).escape(),
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      Author.findById(req.params.id).exec( function(err, author) {
        ...
        Author.findByIdAndUpdate(req.params.id, author, {}, function (err,theauhtor) {
            if (err) { return next(err); }
               // Successful - redirect to book detail page.
               res.redirect(theauhtor.url);
            });
      }
    }
]
